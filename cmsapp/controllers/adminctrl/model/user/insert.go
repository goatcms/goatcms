...
import (
	...
	httpuser "github.com/goatcms/goatcms/cmsapp/http/httpmodel/user"
...
func NewInsert(dp dependency.Provider) (ctrl *Insert, err error) {
	...
	if ctrl.view, err = ctrl.deps.Template.View(goathtml.DefaultLayout, "admin/model/user/insert", nil); err != nil {
		return nil, err
	}
	return ctrl, nil
}
...
func (c *Insert) Get(scope app.Scope) {
	...
	if err := deps.Responser.Execute(c.view, map[string]interface{}{
		...
		"Fields": entities.UserMainFields,
...
func (c *Insert) Post(scope app.Scope) {
	...
	entity, err := httpuser.NewForm(scope, entities.UserMainFields)
	if err != nil {
		c.deps.Logger.ErrorLog("%v", err)
		deps.RequestError.Error(312, err)
		return
	}
	if msgs, err = entities.ValidUser(entity); err != nil {
		c.deps.Logger.ErrorLog("%v", err)
		deps.RequestError.Error(312, err)
		return
	}
	...
	deps.Responser.Redirect("/admin/user")
}