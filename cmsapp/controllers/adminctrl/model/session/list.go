...
type List struct {
	deps struct {
		...
		Finder   dao.SessionSearch  `dependency:"SessionSearch"`
	}
	view *template.Template
}

// NewList create instance of a list session controller
func NewList(dp dependency.Provider) (*List, error) {
	...
	if ctrl.view, err = ctrl.deps.Template.View(goathtml.DefaultLayout, "admin/model/session/list", nil); err != nil {
		return nil, err
	}
	return ctrl, nil
}
...
func (c *List) Get(requestScope app.Scope) {
	var (
		...
		searchParams dao.SessionSearchParams
	)
	if err = requestScope.InjectTo(&requestDeps); err != nil {
		fmt.Println(err)
		return
	}
	if rows, err = c.deps.Finder.Search(requestScope, entities.SessionMainFields, &searchParams); err != nil {
		requestDeps.RequestError.Error(312, err)
		return
	}
	collection := []*entities.Session{}
	for rows.Next() {
	...
	if err = requestDeps.Responser.Execute(c.view, map[string]interface{}{
		...
		"Labels":     entities.SessionMainFields,
	}); err != nil {
		requestDeps.RequestError.Error(312, err)
		return
	}
