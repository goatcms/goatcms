...
import (
	...
	dao "github.com/goatcms/goatcms/cmsapp/dao/sqlite/dao/user"
...
func doUserLoginQuery(t *testing.T) bool {
	var (
		...
		deps           struct {
			...
			Inserter     maindef.Insert         `dependency:"UserInsert"`
	...
	if _, err = deps.Inserter.Insert(s, expectedEntity, entities.UserMainFields); err != nil {
		t.Error(err)
		return false
	}
	if _, err = helpers.Commit(s); err != nil {
